...
- Reactive Programming이 무엇인지 설명하시오.
   절차형 프로그래밍처럼, '이거하고 저거해라' 방식이 아니라, 값의 흐름을 설정해놓고 거기서 어떤 값이 나오면 그 값을 구독하고 있다가 처리해주는 '선언'하는 방식으로 처리해준다. 
   이 값들이 파이프라인에서 거치는 처리 작업들은 함수형 프로그래밍과도 잘 맞기 때문에 반응형 프로그래밍과 함수형 프로그래밍, 선언현 프로그래밍은 보통 같이 간다. 
   반응형 프로그래밍에서 가장 중요한 건, 구현하고자 하는 소프트웨어에서 일어나는 일들을 흐름들로 재해석 할 수 있어야 한다는 점이다. 그 흐름을 observable로 구현한 다음, 
   이를 타고 내려오는 데이터들을 오퍼레이터로 정제하고 그 최종값에 구독자가 어떻게 반응해야할지를 코딩한다. 
   
   소프트웨어의 각종 요소들을 이처럼 순수함수로 가공할 수 있는 스트림(흐름) 으로 다룸으로써 함수형 프로그래밍의 강점들이 적용될 수 있는 범위를 확장해준다. 
