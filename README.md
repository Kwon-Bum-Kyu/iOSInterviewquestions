...
- Reactive Programming이 무엇인지 설명하시오.
   ```
   값의 흐름을 설정해놓고 거기서 어떤 값이 나오면, 그 값을 구독하고 있다가 처리해주는 방식. 
   반응형 프로그래밍에서 중요한 건, 구현하고자 하는 소프트웨어에서 일어나는 일들을 흐름들로 재해석 할 수 있어야 한다는 점인데, 원하는 흐름을 observable로 구현한 다음에
   이를 타고 내려오는 데아터들을 오퍼레이터로 정제하고, 그 최종값이 구독자가 어떻게 반응해야할지를 코딩해주면 된다. 
   이처럼 데이터를 순수함수로 가공할 수 있는 스트림을 다룸으로써 함수형 프로그래밍의 강점이 적용될 수 있는 범위를 확장해주게 된다. 
   ```
